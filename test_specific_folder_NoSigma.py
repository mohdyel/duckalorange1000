# ==========================================================
# Standalone Inference — OOF reproduction or Fold-Averaged
...
preset_candidates = []
if meta.get("speed_preset"):  # prefer the exact preset used in training, when present
...
if feat is None:
    # final attempt with current defaults (max) just to report what we got
...
# ---------------- Predict ----------------
if MODE == "oof_repro":
    preds = predict_oof_for_training_planet(PLANET_ID, Xs, models_bundle, planets_list)
elif MODE == "ensemble":
    preds = predict_ensemble_all_folds(Xs, models_bundle)
else:
    raise ValueError("MODE must be 'oof_repro' or 'ensemble'.")

# ---------------- Save train-like row ----------------
out_df = pd.DataFrame([preds], columns=wl_cols)
